...
# Calculate the exact total frames to stretch the fractal zoom perfectly from start to finish
END_PTS=$(awk "BEGIN {print int($DURATION * 25)}")

echo "Video duration set to: $DURATION seconds (Total Frames: $END_PTS)"
...
if [ -f "$AUDIO_FILE" ]; then
    ...
    echo "No audio found. Procedurally generating Infinite Hall of Mirrors..."
    ...
    # 1. crop=iw/2:ih:0:0 -> Corta a tela exatamente no meio (pegando o lado esquerdo).
    # 2. split -> Duplica essa metade.
    # 3. hflip -> Espelha a cópia horizontalmente.
    # 4. hstack -> Cola o lado esquerdo com o lado espelhado, criando um caleidoscópio simétrico perfeito.
    
    ffmpeg -y \
        -t "${DURATION}" \
        -f lavfi -i "mandelbrot=s=1920x1080:maxiter=150:start_x=${START_X}:start_y=${START_Y}:end_pts=${END_PTS}:end_scale=0.00000000001" \
        -filter_complex \
        "[0:v]crop=iw/2:ih:0:0,split[left][left_copy]; \
         [left_copy]hflip[right]; \
         [left][right]hstack[mirrored]; \
         [mirrored]ass='${ASS_FILE}'[v]" \
        -map "[v]" \
        -c:v libx264 \
        -preset ultrafast \
        -crf 28 \
...
if [ -f "$AUDIO_FILE" ]; then
    ...
    ffmpeg -y \
        -t "${DURATION}" \
        -f lavfi -i "mandelbrot=s=1920x1080:maxiter=150:start_x=${START_X}:start_y=${START_Y}" \
        -vf "ass='${ASS_FILE}'" \
        -c:v libx264 \
        -preset ultrafast \
        -crf 28 \
